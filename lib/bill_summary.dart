...
class _BillSummaryPageState extends State<BillSummaryPage> {
  ...
  Widget build(BuildContext context) {
    ...
    final totalAmount = widget.subTotal +
        gst +
      ...
      body: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            // Clothing Type (if selected)
            if (widget.clothingType != null) ...[
              Text(
                'Clothing Type: ${widget.clothingType} - ₹${widget.clothingPrices}',
                style: const TextStyle(fontSize: 16),
              ),
              const SizedBox(height: 16),
            ],

            // Turf Booking (if selected)
            if (widget.turfHours > 0) ...[
              Text(
                'Turf Booking for ${widget.turfHours} hour(s): ₹${widget.turfCharge}',
                style: const TextStyle(fontSize: 16),
              ),
              const SizedBox(height: 16),
            ],

            // Membership Type (if selected)
            if (widget.membershipType != null) ...[
              Text(
                'Membership Type: ${widget.membershipType} - ₹${widget.membershipCharge}',
                style: const TextStyle(fontSize: 16),
              ),
              const SizedBox(height: 16),
            ],
